/**
 * Complex snapshot tests — auto-discovers tests/scripts/complex/test_*.py,
 * starts a DH server per test, renders, and compares the accessibility-tree
 * snapshot against files in tests/snapshots/complex/.
 *
 ...
 *
 * Run:     npx vitest run --project snapshots -t 'complex'
 * Update:  npx vitest run --project snapshots -t 'complex' --update
 */
...
const entries = discoverPyFiles(REPO_ROOT, 'tests/scripts/complex', 'test_');
...
describe.runIf(globalThis.__DH_SERVER_AVAILABLE__ && entries.length > 0)('complex snapshots', () => {
        ...
        it(name, async () => {
            ...
            try {
                ...
                await expect(snap).toMatchFileSnapshot(
                    join(SNAPSHOTS_DIR, 'complex', `${name}.snap`)
                );
            } finally {
                await server.kill();
            }
