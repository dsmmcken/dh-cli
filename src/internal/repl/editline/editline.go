...
import (
	...
	"github.com/dsmmcken/dh-cli/src/internal/repl/editline/internal/textarea"
...
// applySearchInputStyles applies the given search input style to the
// hctrl.pattern textinput using the v2 SetStyles API.
func (m *Model) applySearchInputStyles(si struct {
	PromptStyle      lipgloss.Style
	TextStyle        lipgloss.Style
	BackgroundStyle  lipgloss.Style
	PlaceholderStyle lipgloss.Style
	...
	s := m.hctrl.pattern.Styles()
	s.Focused = textinput.StyleState{
		Prompt:      si.PromptStyle,
		Text:        si.TextStyle,
		Placeholder: si.PlaceholderStyle,
	}
	s.Blurred = s.Focused
	m.hctrl.pattern.SetStyles(s)
}
...
func (m *Model) Focus() tea.Cmd {
	...
	m.updatePrompt()
	m.applySearchInputStyles(m.FocusedStyle.SearchInput)
...
func (m *Model) Blur() {
	...
	m.text.Blur()
	m.applySearchInputStyles(m.BlurredStyle.SearchInput)
}
...
func (m *Model) hidePrompt(b bool) {
	...
	if b {
		...
		// v2 textinput does not expose cursor mode directly;
		// blurring the pattern input hides the cursor.
	} else {
...
func (m *Model) handleSearching(imsg tea.Msg) (stillSearching bool, restMsg tea.Msg, cmd tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		default:
			if !msg.Mod.Contains(tea.ModAlt) && (msg.Code == tea.KeySpace ||
